...
namespace FCD.Tests
{
    public abstract class FCDTestBase : AbpIntegratedTestBase<FCDTestModule>
    {
        ...
        protected void UsingDbContext(Action<FCDDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FCDDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FCDDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FCDDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FCDDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FCDDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FCDDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FCDDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FCDDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FCDDbContext, Task<T>> func)
        {
            T result;
